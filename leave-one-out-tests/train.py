...
def get_groups_array_targets(pairs):
    ...
    pair_simplified_ec_numbers = pd.merge(pairs_df, ec_unique, on='target', how='left')
    pair_simplified_ec_numbers = pair_simplified_ec_numbers.drop(['target','drug'], axis=1)

    df_groups, ec_group_dict = assign_groups(pair_simplified_ec_numbers, 'simplified_ec_number')  
...
def excluded_group_cv(pairs_all, classes_all, embedding_df, clfs, type):
    pairs, classes = balance_data(pairs_all, classes_all)
    if type == 'ec':
        groups, group_dict = get_groups_array_targets(pairs)
    elif type == 'atc':
...
#all_scores_df = group_cv(pairs, labels, embeddings, clfs, 'atc')
